...
export default function ViewerDashboard() {
    ...
    const fetchDashboardData = async (appliedFilters = {}) => {
        ...
        try {
            ...
            const todayExpenses = expenses.filter((e) => e.createdAt?.split("T")[0] === todayDate);

            const sumAmount = (arr) => arr.reduce((sum, e) => sum + Number(e.amount || 0), 0);
            ...
            setData({
                ...
                totalAmount: sumAmount(pending) + sumAmount(approvedList) + sumAmount(rejected) + sumAmount(hold),
    ...
    const buildLink = (base) => `${base}?${new URLSearchParams(filters).toString()}`;

    const requestCards = [
        { title: "Total Requests", count: data.totalRequests, amount: data.totalAmount, color: "#4B49AC", icon: "bi-collection", link: buildLink("/viewer/allExpenses") },
        { title: "Pending Requests", count: data.pendingRequests, amount: data.pendingAmount, color: "#FFC107", icon: "bi-hourglass-split", link: buildLink("/viewer/allPendingExpenses") },
        { title: "Approved Requests", count: data.approvedRequests, amount: data.approvedAmount, color: "#20C997", icon: "bi-check-circle", link: buildLink("/viewer/allApprovedExpenses") },
        { title: "Rejected Requests", count: data.rejectedRequests, amount: data.rejectedAmount, color: "#FF6B6B", icon: "bi-x-circle", link: buildLink("/viewer/allRejectedExpenses") },
        { title: "In-Process", count: data.inProcessRequests, amount: data.holdAmount, color: "#4D96FF", icon: "bi-arrow-repeat", link: buildLink("/viewer/allHoldExpenses") },
        { title: "Today’s New Requests", count: data.todayRequests, amount: data.todayAmount, color: "#00B8D9", icon: "bi-calendar-event", link: buildLink("/viewer/todayRequests") },
    ];

    const roleCards = [
        { title: "Total Users", value: data.totalUsers, color: "#6F42C1", icon: "bi-people", link: "/viewer/manageEmployee" },
        { title: "Facility Managers", value: data.totalFacilityManagers, color: "#198754", icon: "bi-person-badge", link: "/viewer/manageEmployee" },
        { title: "CLMs", value: data.totalCLMs, color: "#FD7E14", icon: "bi-building", link: "/viewer/manageEmployee" },
        { title: "Zonal Heads", value: data.totalZonalHeads, color: "#0D6EFD", icon: "bi-diagram-3", link: "/viewer/manageEmployee" },
        { title: "Business Finance", value: data.totalBusinessFinance, color: "#6610f2", icon: "bi-currency-rupee", link: "/viewer/manageEmployee" },
        { title: "Procurement", value: data.totalProcurement, color: "#0dcaf0", icon: "bi-cart-check", link: "/viewer/manageEmployee" },
        { title: "PR / PO", value: data.totalPrPo, color: "#adb5bd", icon: "bi-receipt", link: "/viewer/manageEmployee" },
        { title: "Zonal Commercial", value: data.totalZonalCommercial, color: "#20c997", icon: "bi-briefcase", link: "/viewer/manageEmployee" },
        { title: "Missing Bridge", value: data.totalMissingBridgeUsers, color: "#dc3545", icon: "bi-exclamation-octagon", link: "/viewer/manageEmployee" },
    ];
    ...
    return (
        <main id="main" className="main">
            <div className="d-flex justify-content-between align-items-center mb-3">
                <PageTitle child="Viewer Dashboard" />
                ...
                <div className="card-body">
                    ...
                    <div className="row g-3 align-items-end">
                        ...
                        <div className="col-md-2">
                            ...
                            <select className="form-select" value={filters.month} onChange={(e) => handleFilterChange("month", e.target.value)}>
                                <option value="">Select Month</option>
                                {["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"].map((m, j) => (<option key={j} value={j + 1}>{m}</option>))}
                            </select>
                        </div>
                        <div className="col-md-2">
                            ...
                            <select className="form-select" value={filters.year} onChange={(e) => handleFilterChange("year", e.target.value)}>
                                ...
                                {Array.from({ length: 6 }, (_, i) => { const y = 2023 + i; return (<option key={y} value={y}>{y}</option>); })}
                            </select>
                        </div>
            ...
            {!load && (
                <>
                                    ...
                                    <div className="card info-card h-100">
                                        <div className="card-body">
                                            ...
                                            <div className="d-flex align-items-center">
                                                <div className="card-icon text-white rounded-circle d-flex align-items-center justify-content-center me-3" style={{ width: 50, height: 50, background: c.color }}>
                                                    <i className={`bi ${c.icon} fs-4`} />
                                                </div>
                                                <div>
                                                    <h6 className="fw-bold mb-0">{c.count}</h6>
                                                    <small className="text-muted">₹ {c.amount.toLocaleString()}</small>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <h4 className="mt-4 mb-3 fw-bold" style={{ color: "#012970" }}>System & Role Overview</h4>
                    <div className="row g-3">
                        {roleCards.map((c, i) => (
